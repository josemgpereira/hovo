...
class Leave extends CI_Controller
{
    ...
    public function Add_Applications()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $appstartdate = $this->input->post('startdate');
            $appenddate   = $this->input->post('enddate');
            if(empty($appenddate)) {$appenddate = $appstartdate;}
	...
	public function HOLIvalueDelete($id)
	{
		if ($this->session->userdata('user_login_access') != False) {
			$success = $this->leave_model->DeletHoliday($id);
        ...
        if (is_array($report) || is_object($report))
        {
            foreach ($report as $value) {
                ...
                echo "<tr>
                        ...
                        <td>$value->name</td>
                        <td>$value->leave_duration horas</td>
    ...
    public function LeaveAssign()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if (!empty($leaveID)) {
                ...
                if(empty($earnTaken)) {
					$totalday   = 'Saldo: 0 dias';
				} else {
					$totalday   = 'Saldo: '.($earnTaken->hour / 8).' dias';
				}
                echo $totalday;       
                }else {
                //$totalday   = $leavetypes->leave_day . '/' . ($daysTaken/8);
				if($leavetypes->leave_day - $daysTakenval <= 0) {
					$totalday   = 'Saldo: 0 dias';
				} else {
					$totalday   = 'Saldo: '.($leavetypes->leave_day - $daysTakenval).' dias de '.$leavetypes->leave_day;
				}
                echo $totalday;
                }
